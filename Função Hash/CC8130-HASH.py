...
# Lista de valores hash fornecidos
hashes_fornecidos = [
...
# Função para calcular os valores hash
def calcular_hashes(frase):
    sha256 = hashlib.sha256(frase.encode()).hexdigest()
    md5 = hashlib.md5(frase.encode()).hexdigest()
    return sha256, md5

# Comparando os valores hash
for i, frase in enumerate(frases):
    sha256, md5 = calcular_hashes(frase)
    sha256_fornecido, md5_fornecido = hashes_fornecidos[i]
    print(f"Frase {i+1}:")
    print(f"  SHA256 correto: {sha256 == sha256_fornecido}")
    print(f"  MD5 correto: {md5 == md5_fornecido}")
